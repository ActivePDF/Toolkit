    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            using (APToolkitNET.Toolkit toolkit = new APToolkitNET.Toolkit(toolkitPath))
            {
                ...
                // Expands or collapses the bookmark tree initial view when
                // opened.
                bookmarkManager.Expanded = true;

                // Create the new PDF file
                int result = toolkit.OpenOutputFile($"{strPath}Toolkit.ExpandBookmarks.pdf");
                if (result == 0)
                {
