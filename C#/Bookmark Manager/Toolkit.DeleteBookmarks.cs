    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            using (APToolkitNET.Toolkit toolkit = new APToolkitNET.Toolkit(CoreLibPath: toolkitPath))
            {
                ...
                int result = toolkit.OpenOutputFile(FileName: $"{strPath}Toolkit.DeleteBookmarks.pdf");
                if (result == 0)
                {
                    ...
                    if (result == 0)
                    {
                        // You can delete bookmarks based on ID, title, font
                        // color, and font style. This example will delete all
                        // bookmarks with a black font color.
                        bookmarkManager.Delete(
                            Key: APToolkitNET.BookmarkFilter.BookmarkColor,
                            Value: "#000000");
