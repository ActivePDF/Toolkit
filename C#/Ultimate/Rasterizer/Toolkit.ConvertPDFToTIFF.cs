        ...
        static void Main(string[] args)
        {
            ...
            using (APToolkitNET.Toolkit toolkit = new APToolkitNET.Toolkit(toolkitPath))
            {
                ...
                if (result == 0)
                {
                    ...
                    string outputFile = $"{strPath}ConvertToTIFF.tif";

                    // Convert the input PDF to a TIFF image
                    if (!rasterizer.ToImage(sFileName: $"{outputFile}", eImageType: APToolkitNET.APImageType.TIFF))
                    {
