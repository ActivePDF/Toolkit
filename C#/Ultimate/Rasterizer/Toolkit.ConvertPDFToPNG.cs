        ...
        static void Main(string[] args)
        {
            ...
            using (APToolkitNET.Toolkit toolkit = new APToolkitNET.Toolkit(toolkitPath))
            {
                ...
                if (result == 0)
                {
                    ...
                    {
                        string outputFile = $"{strPath}ConvertToPNG.{currentPage}.png";

                        // Convert each page of the input PDF to a PNG
                        if (!rasterizer.ToImage(sFileName: $"{outputFile}", eImageType: APToolkitNET.APImageType.PNG, currentPage))
                        {
