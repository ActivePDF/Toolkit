...
intResult = oTK.OpenOutputFile(strPath & "Toolkit.PageLabels.pdf")
If intResult = 0 Then
    ' Open the template PDF
    intResult = oTK.OpenInputFile(strPath & "Toolkit.Input.pdf")
    If intResult = 0 Then
        ' Create a cover page
        strText = "ActivePDF Toolkit"
        oTK.NewPage
        oTK.SetFont "Helvetica", 72
        textWidth = oTK.GetTextWidth(strText)
        textHeight = oTK.GetTextHeight(strText)
        oTK.PrintText (612 - textWidth) / 2, (792 - textHeight) / 2, strText

        ' Add a page label to the cover page
        oTK.SetOutputPageLabels 1, 1, 0, "Cover", 1

        ' Add numbered page labels to remaining pages
        oTK.SetOutputPageLabels 2, 3, 3, "", 1
